...
@section('contenido')
    <div class="container mx-auto md:flex">
        ...
        <div class="md:w-1/2 p-5">
            ...
            <div class="p-5 shadow bg-white mb-5">
                @auth
                    <p class="text-xl font-bold text-center mb-4">Agrega un Nuevo Comentario</p>

                    @if (session('mensaje'))
                        <div class="bg-green-500 p-2 rounded-r-lg mb-6 text-white text-center uppercase font-bold">
                            {{ session('mensaje') }}
                        </div>
                    @endif
                    ...
                    <form action="{{ route('comentarios.store', ['post' => $post, 'user' => $user]) }}" method="POST">
                        ...
                        <div class="mb-5">
                            ...
                            <textarea id="comentario" name="comentario" placeholder="Agrega un Comentario"
                                class="border px-3 py-2 w-full rounded-lg @error('comentario') border-red-500 @enderror"></textarea>
                            @error('comentario')
                    ...
                    @if ($post->comentarios->count())
                        @foreach ($post->comentarios as $comentario)
                            <div class="p-5 border-gray-300 border-b">
                                ...
                                <p>{{ $comentario->cometario }}</p>
                                <p class="text-sm text-gray-500">{{ $comentario->created_at->diffForHumans() }}</p>

                            </div>
                        @endforeach
                    @else
                        <p class="p-18 text-center">No Hay Comentarios Aún</p>
                    @endif
                </div>
